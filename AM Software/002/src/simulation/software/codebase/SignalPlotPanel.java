...
public class SignalPlotPanel extends JPanel {
    ...
    private void drawTimePlot(Graphics g) {
        ...
        for (int i = 0; i < time.length - 1; i++) {
            int x1 = margin + (int) (((time[i] / time[time.length - 1]) * (width - 2 * margin) + panTime) * zoomTime);
            int x2 = margin + (int) (((time[i + 1] / time[time.length - 1]) * (width - 2 * margin) + panTime) * zoomTime);
            int y1 = height / 2 - (int) (message[i] * (height / 4));
            int y2 = height / 2 - (int) (message[i + 1] * (height / 4));
        ...
        for (int i = 0; i < time.length - 1; i++) {
            int x1 = margin + (int) (((time[i] / time[time.length - 1]) * (width - 2 * margin) + panTime) * zoomTime);
            int x2 = margin + (int) (((time[i + 1] / time[time.length - 1]) * (width - 2 * margin) + panTime) * zoomTime);
            int y1 = height / 2 - (int) (carrier[i] * (height / 4));
            int y2 = height / 2 - (int) (carrier[i + 1] * (height / 4));
            if (x1 >= margin && x1 <= width - margin && x2 >= margin && x2 <= width - margin) {
                g2d.drawLine(x1, y1, x2, y2);
            }
        }
        ...
        for (int i = 0; i < time.length - 1; i++) {
            int x1 = margin + (int) (((time[i] / time[time.length - 1]) * (width - 2 * margin) + panTime) * zoomTime);
            int x2 = margin + (int) (((time[i + 1] / time[time.length - 1]) * (width - 2 * margin) + panTime) * zoomTime);
            int y1 = height / 2 - (int) (modulatedSignal[i] * (height / 4));
            int y2 = height / 2 - (int) (modulatedSignal[i + 1] * (height / 4));
            if (x1 >= margin && x1 <= width - margin && x2 >= margin && x2 <= width - margin) {
                g2d.drawLine(x1, y1, x2, y2);
            }
        }
        ...
        if (demodulatedSignal != null && demodulatedSignal.length > 0 && demodulatedSignal[0] != 0) {
            ...
            for (int i = 0; i < time.length - 1; i++) {
                int x1 = margin + (int) (((time[i] / time[time.length - 1]) * (width - 2 * margin) + panTime) * zoomTime);
                int x2 = margin + (int) (((time[i + 1] / time[time.length - 1]) * (width - 2 * margin) + panTime) * zoomTime);
                int y1 = height / 2 - (int) (demodulatedSignal[i] * (height / 4));
                int y2 = height / 2 - (int) (demodulatedSignal[i + 1] * (height / 4));
                if (x1 >= margin && x1 <= width - margin && x2 >= margin && x2 <= width - margin) {
                    g2d.drawLine(x1, y1, x2, y2);
                }
            }
    ...
    private void drawFreqPlot(Graphics g) {
        ...
        for (int i = 0; i < frequency.length - 1; i++) {
            int x1 = margin + (int) (((frequency[i] / frequency[frequency.length - 1]) * (width - 2 * margin) + panFreq) * zoomFreq);
            int x2 = margin + (int) (((frequency[i + 1] / frequency[frequency.length - 1]) * (width - 2 * margin) + panFreq) * zoomFreq);
            int y1 = height - margin - (int) (spectrum[i] / maxSpectrum * (height / 2 - margin));
            int y2 = height - margin - (int) (spectrum[i + 1] / maxSpectrum * (height / 2 - margin));
            if (x1 >= margin && x1 <= width - margin && x2 >= margin && x2 <= width - margin) {
                g2d.drawLine(x1, y1, x2, y2);
            }
        }
        ...
        for (int i = 1; i < frequency.length - 1; i++) {
            if (spectrum[i] > spectrum[i - 1] && spectrum[i] > spectrum[i + 1] && spectrum[i] / maxSpectrum > 0.1) {
                int x = margin + (int) (((frequency[i] / frequency[frequency.length - 1]) * (width - 2 * margin) + panFreq) * zoomFreq);
                int y = height - margin - (int) (spectrum[i] / maxSpectrum * (height / 2 - margin));
                if (x >= margin && x <= width - margin) {
                    g2d.drawString(String.format("%.0f Hz", frequency[i]), x + 5, y - 5);
                }
            }
        }
    }
